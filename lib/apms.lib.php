...
// BS3 Style
function apms_paging($write_pages, $cur_page, $total_page, $url, $add='', $first='<i class="fa fa-angle-double-left"></i>', $prev='<img src="'.G5_IMG_URL.'/icon_page_prev.png"/>', $next='<img src="'.G5_IMG_URL.'/icon_page_next.png"/>', $last='<i class="fa fa-angle-double-right"></i>') {
...
function apms_widget($wname, $wid='', $opt='', $mopt='', $wdir='', $addon=''){
	...
	if($wdir) {
		...
		if($addon) { // 애드온
			...
			$widget_path = G5_SKIN_PATH.'/addon/'.$wname;			/*			여기집중해서 보기				*/
		} else {
